...
exports.login = catchAsync(async (req, res, next) => {
  ...
  //we cannot modify or delete this cookie to log the user out. The solution is to send a new cookie, with no token, that will override the current cookie. 
  //We’ll set this cookie to expire after a very short period of time, effectively deleting the cookie
...
exports.protect = catchAsync(async(req,_res,next) => {
    ...
    if (
      ...
      token = req.cookies.jwt;//storing as cookie
    }
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      //PUG part--Its like passing data into templates
      res.locals.user = currentUser;
      return next();
    } catch (err) {
      return next();
